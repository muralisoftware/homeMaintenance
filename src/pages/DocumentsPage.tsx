  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-darkblue-900">Document Manager</h2>
          <p className="text-sm text-darkblue-500 mt-0.5">Securely store important documents and receipts</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            !filter ? 'bg-gold-50 text-gold-700 border border-gold-200' : 'bg-white text-darkblue-500 border border-darkblue-200 hover:bg-darkblue-50'
          }`}
        >
          All
        </button>
        {DOC_TYPES.map((t) => (
          <button
            ...
            onClick={() => setFilter(t)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              filter === t ? 'bg-gold-50 text-gold-700 border border-gold-200' : 'bg-white text-darkblue-500 border border-darkblue-200 hover:bg-darkblue-50'
            }`}
          >
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-darkblue-100">
              <h3 className="text-lg font-semibold text-darkblue-900">{editingId ? 'Edit Document' : 'Add Document'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-darkblue-700 mb-1">Title</label>
                <input
                  type="text"
                  value={form.title}
              ...
              <div>
                <label className="block text-sm font-medium text-darkblue-700 mb-1">Document Type</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-darkblue-700 mb-1">File URL / Link</label>
                <input
                  type="url"
                  value={form.file_url}
              ...
              <div>
                <label className="block text-sm font-medium text-darkblue-700 mb-1">File Type</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-darkblue-700 mb-1">Expiry Date (optional)</label>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-xl border p-4 hover:shadow-md transition-shadow ${
                  isExpired ? 'border-red-200' : isExpiring ? 'border-amber-200' : 'border-darkblue-200'
                }`}
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-1 rounded-lg hover:bg-darkblue-100 text-darkblue-300 hover:text-gold-600 transition-colors"
                    >
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => handleDelete(doc.id)}
                      className="p-1 rounded-lg hover:bg-darkblue-100 text-darkblue-300 hover:text-red-500 transition-colors"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
                <h4 className="text-sm font-medium text-darkblue-900 mb-1">{doc.title}</h4>
                <p className="text-xs text-darkblue-500 mb-3">
                ...
                {doc.expiry_date && (
                  <div className={`text-xs px-2 py-1 rounded-lg inline-flex items-center gap-1 ${
                    isExpired ? 'bg-red-50 text-red-600' : isExpiring ? 'bg-amber-50 text-amber-600' : 'bg-darkblue-50 text-darkblue-500'
                  }`}>
                ...
                {doc.file_url && (
                  <a
                    ...
                    className="mt-2 text-xs text-gold-600 hover:text-gold-700 font-medium flex items-center gap-1"
                  >
                ...
                {doc.notes && (
                  <p className="text-xs text-darkblue-400 mt-2 line-clamp-2">{doc.notes}</p>
                )}
              </div>
            );
          })}
